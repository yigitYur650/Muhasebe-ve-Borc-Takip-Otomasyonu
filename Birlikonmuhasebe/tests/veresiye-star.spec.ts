...
test.describe('Veresiye Müşteri Yıldızlama Sistemi', () => {
  ...
  test('yıldızlı müşteriler listenin en üstünde gösterilmelidir', async ({ page }) => {
    ...
    await page.getByPlaceholder('admin@sirket.com').fill(testEmail);
    await page.getByPlaceholder('••••••••').fill(testPassword); 
    await page.getByRole('button', { name: 'Giriş Yap' }).click(); 

    // Giriş yapıldıktan sonra sayfanın yüklenmesini garantilemek için bekliyoruz
    await expect(page.getByText('Ciro Takip')).toBeVisible({ timeout: 10000 });
